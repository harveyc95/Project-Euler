...
int value (int i, int j, int** input, int size);
...
int main () {
 ...
 //    
    ...
    int *array[20] = {a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t};
    ...
    for (int i = 0; i < 20; i++) {
    	for (int j = 0; j < 20; j++) {
    		unsigned int prod[4] = {1, 1, 1, 1};

    		for (int k = 0; k < maxAdjacent; k++) {
    			prod[0] *= value(i, j + k, array, 20);
    		}
    		// Check Vertical
    		for (int k = 0; k < maxAdjacent; k++) {
    			prod[1] *= value(i + k, j, array, 20);
    		}
    		// Check Right Diagonal
    		for (int k = 0; k < maxAdjacent; k++) {
    			prod[2] *= value(i - k, j - k, array, 20);
    		}
    		// Check Left Diagonal
    		for (int k = 0; k < maxAdjacent; k++) {
    			prod[3] *= value(i + k, j - k, array, 20);
    		}
    		for (int k = 0; k < 4; k++) {
    			largestProduct = (prod[k] > largestProduct) ? prod[k] : largestProduct;
    		}
    	}
    }
    printf("%u\n", largestProduct);
